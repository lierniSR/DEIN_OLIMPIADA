...
public class ControladorDeportista implements Initializable {
    ...
    public void cambiarEvento(ActionEvent actionEvent) {
        ...
        try {
            scene = new Scene(fxmlLoader.load(), 767, 502);
            stagePrimario.setTitle("Gestion de Eventos");
            stagePrimario.getIcons().add(new Image(String.valueOf(OlimpiadaPrincipal.class.getResource("Imagenes/Eventos/icono.png"))));
            stagePrimario.setResizable(false);
            stagePrimario.setScene(scene);
            stagePrimario.show();

            ControladorEvento controladorEvento = fxmlLoader.getController();
            controladorEvento.setStage(stagePrimario);
        } catch (IOException e) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText(null);
            alert.setTitle("FXML");
    ...
    public void aniadirDeportista(ActionEvent actionEvent) {
        ...
        FXMLLoader loader = new FXMLLoader(OlimpiadaPrincipal.class.getResource("FXML/Deportista/aniadirDepor.fxml"));
        Parent root = null;
        try {
            root = loader.load();
    ...
    public void modificarDeportista(ActionEvent actionEvent) {
        if(tablaDeportistas.getSelectionModel().getSelectedItem() == null){
            ...
            //Esto si el controlador necesita hacer algo en la ventana principal
            // Cargar el FXML de la ventana modal
            FXMLLoader loader = new FXMLLoader(OlimpiadaPrincipal.class.getResource("FXML/Deportista/modificarDepor.fxml"));
            Parent root = null;
            try {
                root = loader.load();
